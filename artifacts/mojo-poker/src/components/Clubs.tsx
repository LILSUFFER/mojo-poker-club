        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24 }}>
          {clubs.map((club, i) => (
            <motion.div
              ...
              onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'rgba(204,21,21,0.35)'; el.style.transform = 'translateY(-3px)'; }}
            ...
            >
              {/* Card top stripe */}
              <div style={{ height: 3, background: 'linear-gradient(to right, hsl(4 80% 45%), hsl(4 80% 60%))' }} />
              ...
              <div style={{ padding: '32px 32px 28px', display: 'flex', flexDirection: 'column', gap: 0, flex: 1 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 24 }}>
                  ...
                  {club.badge && (
                    <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 700, padding: '5px 11px', borderRadius: 6, background: 'rgba(204,21,21,0.1)', border: '1px solid rgba(204,21,21,0.2)', color: 'hsl(4 80% 55%)' }}>
                      {club.badge}
                    </span>
                  )}
                </div>
                ...
                <div style={{ padding: '18px 20px', borderRadius: 10, background: '#0a0c12', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                    <span style={{ fontSize: 32, fontWeight: 900, color: 'hsl(4 80% 48%)', letterSpacing: '0.06em', fontVariantNumeric: 'tabular-nums' }}>{club.clubId}</span>
