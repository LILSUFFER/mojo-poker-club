...
export function VPNSidebar() {
  ...
  const steps = isRu ? [
    'Скачайте VPN (кнопка «С рекламой»)',
    'Отключите Wi-Fi — только мобильный',
    'Выберите страну из списка ниже',
    'Подключитесь к VPN, откройте ClubGG',
    'Профиль → «Выход»',
    '«Зарегистрироваться» — увидите флаг',
  ] : [
    'Download VPN (tap "Continue with Ads")',
    'Turn off Wi-Fi — mobile data only',
    'Pick a country from the list below',
    'Connect VPN, open ClubGG',
    'Profile → "Sign Out"',
    'Tap "Register" — see the country flag',
  ];
  ...
  return (
    <div style={{
      ...
      <div style={{
        borderRadius: 8,
        overflow: 'hidden',
        border: `1px solid ${blueAlpha(0.25)}`,
        background: `linear-gradient(160deg, ${blueAlpha(0.07)} 0%, rgba(255,255,255,0.01) 100%)`,
      }}>
        {/* Header */}
        <div style={{
          padding: '12px 16px',
          background: blueAlpha(0.1),
          borderBottom: `1px solid ${blueAlpha(0.15)}`,
          display: 'flex', alignItems: 'center', gap: 8,
        }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={blue} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>
          </svg>
          <div>
            <p style={{ margin: 0, fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', color: blueAlpha(0.55), textTransform: 'uppercase' }}>
              {isRu ? 'Только для Massiv Union' : 'Massiv Union only'}
            </p>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: blue, lineHeight: 1.3 }}>
              {isRu ? 'Зарубежный флаг в профиле' : 'Foreign flag in profile'}
            </p>
          </div>
        </div>
        ...
        <div style={{ padding: '16px' }}>
          ...
          {/* Divider */}
          <div style={{ height: 1, background: blueAlpha(0.1), marginBottom: 16 }} />

          {/* Download VPN */}
          <div style={{ marginBottom: 16 }}>
            <p style={{ margin: '0 0 8px', fontSize: 9, fontWeight: 700, letterSpacing: '0.12em', color: blueAlpha(0.5), textTransform: 'uppercase' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {countries.map(c => (
                <div key={c.code} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <FlagImg code={c.code} />
                  <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', color: blueAlpha(0.45), textTransform: 'uppercase', minWidth: 20 }}>{c.code}</span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)' }}>{isRu ? c.ru : c.en}</span>
                </div>
              ))}
            </div>
          </div>
